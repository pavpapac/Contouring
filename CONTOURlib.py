...
def Hausdorff(polyA,polyB):
  #Calculates and returns the max,mean and std Hausdorf distance.
  ...
  #First find the slices of polyB that have the same z coordinate as polyA. 
  #These are the slices that we will evaluate the contours for both structures.
...
def HausdorffPoly(poly):
    #FUNCTION HAUSDORFF POLY(): 
    #Given a list of structures (poly) calculates the max, mean and std Haussdorf distance
    # for all combinations (order selection does not matter here).
    #For example calculates Haussdorf for structure 1 with 2,3.. 
    #structure 2 with 3,4..etc etc.
    # Each structure is a lsit of contours outlined by each physician. 
